...
```{r, echo=FALSE}
knitr::include_graphics("your path", dpi = 96)
```
...
To display the saved file as shown in Figure \@ref(fig:sp1), use the `include_graphics()` function from `knitr`.

```{r, sp1, fig.cap="Using knitr::include_graphics()", echo=FALSE}
knitr::include_graphics( "./JPG/YourFileName.jpeg")
```
...
See the `geom_boxplot` <a href="http://docs.ggplot2.org/current/geom_boxplot.html">documentation</a> and the `geom_freqpoly` <a href="http://docs.ggplot2.org/current/geom_histogram.html">documentation</a> for more details.
